		...
		if nextX != x {
			for i := x; i < nextX; i++ {
				if _, ok := locations[i*10000+y]; ok {
					return strconv.Itoa(int(math.Abs(float64(i)) + math.Abs(float64(y))))
				}
		...
		if nextY != y {
			for i := y; i < nextY; i++ {
				if _, ok := locations[x*10000+i]; ok {
					return strconv.Itoa(int(math.Abs(float64(x)) + math.Abs(float64(i))))
				}
...
func main() {
	...
	input := utils.ReadInput("01", "input.txt")
